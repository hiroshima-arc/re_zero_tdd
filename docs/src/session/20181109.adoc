...
*テストおじさん*: つぎに [red]#テストファースト# でやります。
...
Traceback (most recent call last):
  File "replay_main_test.py", line 7, in test_値が3ならばFizzを返すようにする
    self.assertEqual("Fizz", 3)
...
*テストおじさん*: イエス、今やっていることはテストといいながら設計をしています。事実、最初のテスト項目は [red]#TODOリスト# の内容をそのまま転記してますからね。
...
*テストおじさん*: これはunittest.TestCaseクラスを継承したFizzBuzzTestクラスという意味です。
...
*テストおじさん*: これで実装完了・・・とはいきませんね、ロジックは実装して本当にテストがパスするようにしましょう。
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 21, in test_値が5ならばBuzzを返すようにする
...
*テストおじさん*: あー対応する処理がないからテストが失敗してますな。修正修正...
...
*テストおじさん*: ２つ目のテストによってgenerateメソッドの一般化が実現できたわけですがこのようなアプローチを [red]#三角測量# といいます。
...
[source, bash]
----
 $ python replay_main_test.py
 F..
 ======================================================================
 FAIL: test_値が15ならばFizzBuzzを返すようにする (__main__.FizzBuzzTest)
 ----------------------------------------------------------------------
 Traceback (most recent call last):
   File "replay_main_test.py", line 28, in test_値が15ならばFizzBuzzを返すようにする
     self.assertEqual("FizzBuzz", FizzBuzz.generate(15))
...
VSCodeのデバッガを起動してステップ実行で条件分岐を確認するテストおじさん
...
*テストおじさん*: numberが15の場合は最初の判定で処理を抜けてますね。ここはこうかな
...
*テストおじさん*: あ、仕様にそれ以外では値を返すというのがあるのでテストに明記しておきましょう
...
----
 ...
 ----------------------------------------------------------------------
 Traceback (most recent call last):
   File "replay_main_test.py", line 31, in test_値が100ならば100を返すようにする
     self.assertEqual(100, FizzBuzz.generate(100))
...
*テスト書きたい人*: ことろで@staticmethodというのは？
...
*テストおじさん*: 引数に回数を取り戻り値に配列を返すインターフェイスを設計しました。

*テストおじさん*: おわかりいただけだろうか・・・

*テストおじさん*: テストコードと言いながら設計をしているのです、そうテストではなく設計を！
...
*テストおじさん*: ・・・えーと、実装に移りますね。ここはセオリーどおり仮実装から。
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 44, in test_回数を5回実行すると配列を返すようにする
    self.assertEqual([1, 2, "Fizz", 4, "Buzz"], FizzBuzz.iterate(5))
...
*テストおじさん*: あー、generateの引数のnumberが０から始まってるからね
...
*テストおじさん*: ドキュメントがソフトウェアの変更と動機が取れていれば問題ないんですけどねー
...
test_回数を10回実行すると配列を返す (__main__.FizzBuzzTest) ... ok
...
*テストおじさん*: これって実行できる設計ドキュメントみたいなもんですね。

*テストおじさん*: 従来のウォーターフォール型との考え方の違いは [red]#アジャイルソフトウェア開発宣言# という価値から来ているんですけどね。
...
*テストおじさん*: 個人的には [red]#インクリメンタルな設計# も含まれているかな。
...
コメントを書いてはいけないなどと言うつもりはまったくありません。コメントは決して悪い臭いではなく、むしろいい香りなのです。ここでコメントについて言及しているのは、コメントが消臭剤として使われることがあるからです。コメントが非常に丁寧に書かれていたのは、実にわかりにくいコードを補うためだったということがよくあるのです。
____
...
*テストおじさん*: あー、一時変数valueが冗長ですね削除しましょう。
...
*テストおじさん*: ガード節なんですがPythonは言語仕様上インデントしないといけないですが言語によっては１行で書けます。
...
*テストおじさん*: ここでは [red]#条件記述の分解# を適用していましょう。
...
*テストおじさん*: 今回は少し横着をしてテストなしに２つコードを追加しました。ではまとめてテスト・・・ファッ！？
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 40, in test_値が3ならばFizzを返す
    self.assertEqual("Fizz", FizzBuzz.generate(3))
AssertionError: 'Fizz' != 3
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 43, in test_値が5ならばBuzzを返す
    self.assertEqual("Buzz", FizzBuzz.generate(5))
AssertionError: 'Buzz' != 5
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 55, in test_回数を10回実行すると配列を返す
    self.assertEqual([1, 2, "Fizz", 4, "Buzz", "Fizz", 7, 8, "Fizz", "Buzz"], FizzBuzz.iterate(10))
...
----------------------------------------------------------------------
Traceback (most recent call last):
  File "replay_main_test.py", line 52, in test_回数を5回実行すると配列を返す
    self.assertEqual([1, 2, "Fizz", 4, "Buzz"], FizzBuzz.iterate(5))
...
*テスト書いてないとt_wadaさんの前で言った人*: returnが書かれてませんね。

*テストおじさん*: こいつは失礼しました。とまあ、しょぼいミスもテストが教えてくれるわけですね。
...
その意図が明確な名前を付ける。いおうだけなら簡単なことです。ここで、しっかりと胸に刻みつけておいて欲しいのは我々が名前付けを大変重要なことだと思っているということです。
...
*テストおじさん*: 今日のPythonコードも日本語にするとこんな感じ
...
*テストおじさん*: そうっすね、Matzも言ってますし。

https://プログラマが知るべき97のこと.com/%E3%82%A8%E3%83%83%E3%82%BB%E3%82%A4/%E5%90%8D%E5%89%8D%E9%87%8D%E8%A6%81/[名前重要]
...
*テストおじさん*: そして、常に [red]#ベイビーステップ# で進めて行くことに留意しましょう。
